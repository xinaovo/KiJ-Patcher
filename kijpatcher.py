...
G04 leading zeros omitted , absolute positions ,3 integer and 6 decimal *""".format(easyedaVersionString, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
def patchSingleFile(filename, outputPath):
    # Read file by line
    ...
    for p in fileList:
        if(os.path.isfile(os.path.join(gerberFilesDir, p))):
            if(p.endswith(fileFilter)):
                ...
                patchSingleFile(os.path.join(gerberFilesDir, p), os.path.join(os.getcwd(), patchedFilesPath))
                fileCount += 1
